...
function CloudReader:syncAllSdr()
    ...
    for book_id, local_info in pairs(local_sdrs) do
        ...
        if not server_sdr then
            ...
            local local_page = tonumber(local_info.last_page) or 0
            local local_pct = tonumber(local_info.percent) or 0
            local server_page = tonumber(server_sdr.last_page) or 0
            local server_pct = tonumber(server_sdr.percent_finished) or 0

            if local_page > server_page or local_pct > server_pct then
                if self:uploadSdr(book_id, local_info.sdr_path) then
                    uploaded = uploaded + 1
                end
            elseif server_page > local_page or server_pct > local_pct then
                if self:downloadSdr(book_id, local_info.sdr_path) then
                ...
                if file:match("^metadata%..*%.lua$") and not file:match("%.old$") then
                    ...
                    if file_attr then
                        ...
                        if f then
                            ...
                            f:close()
                            local p = content:match('%["last_page"%]%s*=%s*(%d+)')
                            local pct = content:match('%["percent_finished"%]%s*=%s*([%d%.]+)')
                            if p then last_page = tonumber(p) or 0 end
                            if pct then percent = tonumber(pct) or 0 end
                        end
                    end
                end
            end
