...
async fn cmd_serve(
    ...
    // Sync libraries from config to database
    for lib_config in &config.libraries {
        let path = lib_config.path.to_string_lossy().to_string();

        if let Some(existing) = db.get_library_by_name(&lib_config.name)? {
            ...
            if existing.path != path {
                ...
                db.update_library_path(&lib_config.name, &path)?;
            }
        } else {
            // Create new library
            tracing::info!(
